...
pacman::p_load(httr, readxl, here, dplyr, janitor, tidyverse, data.table, scales, gganimate)
```
...
animate_dt <- other_coal_dt %>%
  ...
  mutate(as.numeric(other_coal_dt$year))
  
  
  
  

plot4.9 <-ggplot(other_coal_dt) +
            aes(x=date, y=total, col = coal_destination_country, label = coal_destination_country) +
            geom_point() +
            geom_line(stat="identity") +
            theme_light() +
            ...
            scale_y_continuous(labels = scales::comma) +
            labs(title = "year: {frame_time}", x = 'Year', y = 'Total Coal Exported') +
            transition_time(year, quarter) +
            ease_aes('linear')
